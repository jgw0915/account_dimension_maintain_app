...
duplicate_small_cat = ('耗材/消耗品','RD Material/Sample','Tooling','水電費','旅費','廠辦租金')
...
ws = wb.create_sheet('Organized_SPDG_BY_CAT_Tree')
row_count = 1
for node in Catgory_Tree.traverse_depth_first():
    if node.level != 4:
        ws['{}'.format(COL_Map[node.level]+str(row_count))] = node.data['CN_Alias']
        # ws['{}'.format(COL_Map[node.level]+str(row_count+1))] = node.data['member_name']
    else:
...
for row in ws.iter_rows():
    for cell in row:
        cell.font = font
        cell.border = border
wb.save(r'C:\Users\IEC1-PICST-09\Python_Project\Small Apllication\export file\費用科目列表_v3_SPDG_By_Cat_Tree.xlsx')