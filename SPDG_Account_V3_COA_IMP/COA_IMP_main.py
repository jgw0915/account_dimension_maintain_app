...
ws = wb.create_sheet('Organized_COA_IMP')
row_count = 1
for node in COA_Tree.traverse_depth_first():
    if type(node.data) is not type({}):
        if type(node.children[0].data) is not type({}):
...
for row in ws.iter_rows():
    for cell in row:
        cell.font = font
        cell.border = border
wb.save(r'C:\Users\IEC1-PICST-09\Python_Project\Small Apllication\export file\費用科目列表_v3_COA_IMP.xlsx')